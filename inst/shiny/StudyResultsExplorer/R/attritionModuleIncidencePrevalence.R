...
attritionIncidencePrevalenceUI <- function(id,data) {
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        div(
          style = style,
          pickerInput(
            inputId = ns("denominator_age_group_1"),
            label = "Denominator age group",
            choices = unique(data$denominator_age_group),
            selected = unique(data$denominator_age_group)[1],
            options = list(`actions-box` = TRUE, size = 10, `selected-text-format` = "count > 0"),
            multiple = FALSE
          )
        ),
        div(
          style = style,
          pickerInput(
            inputId = ns("denominator_sex_1"),
            label = "Denominator sex",
            choices = unique(data$denominator_sex),
            selected = unique(data$denominator_sex)[1],
            options = list(`actions-box` = TRUE, size = 10, `selected-text-format` = "count > 0"),
            multiple = FALSE
          )
        ),
      ...
      tabPanel(
        ...
        div(
          style = style,
          pickerInput(
            inputId = ns("denominator_age_group_2"),
            label = "Denominator age group",
            choices = unique(data$denominator_age_group),
            selected = unique(data$denominator_age_group)[1],
            options = list(`actions-box` = TRUE, size = 10, `selected-text-format` = "count > 0"),
...
attritionIncidencePrevalenceServer <- function(id,data) {
  ...
  moduleServer(id, function(input,output,session) {
    ...
    get_attritionTable_data <- reactive({
      ...
      data %>% 
        ...
        filter(denominator_age_group %in% input$denominator_age_group_1) %>%
        filter(denominator_sex %in% input$denominator_sex_1)
    ...
    get_attritionTable_diagram_data <- reactive({
      data %>%
        ...
        filter(denominator_age_group %in% input$denominator_age_group_2) %>%
        filter(denominator_sex %in% input$denominator_sex_2)
