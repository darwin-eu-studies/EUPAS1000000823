...
# indications_module.R
charIndicationsUI <- function(id, show_filters = TRUE, data = NULL, width_px = 200) {
  ns <- NS(id)
...
charIndicationsServer <- function(id, data_indication, filters = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    output$downloadTable_tidy_indications <- downloadHandler(
      filename = function() paste("charIndications_tidy_table_", Sys.Date(), ".xlsx", sep = ""),
      content = function(file) {
        summaryTable <- get_data()
        req(nrow(summaryTable) > 0)
        ft <- DrugUtilisation::tableIndication(summaryTable, type = "flextable", header = "cdm_name")
        exportMultipleFlextablesToExcel(ft_list = list("charIndications" = ft), file = file)
      }
    )
  })
}
...
# ---- RUN ----
# shinyApp(
#   ui = fluidPage(charIndicationsUI("charIndications", data = charIndications)),
#   server = function(input, output, session) {
#     charIndicationsServer("charIndications",data=charIndications)
#   }
# )
