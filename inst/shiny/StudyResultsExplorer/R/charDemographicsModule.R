...
# demographics_module.R
charDemographicsUI <- function(id, show_filters = TRUE, data = NULL, width_px = 200) {
  ns <- NS(id)
...
charDemographicsServer <- function(id, data, filters = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    output$downloadTable_tidy_demographics <- downloadHandler(
      filename = function() paste("charDemographics_tidy_table_", Sys.Date(), ".xlsx", sep = ""),
      content = function(file) {
        ...
        exportMultipleFlextablesToExcel(ft_list = list("charDemographics" = ft), file = file)
      }
    )
  })
}
...
# ---- RUN ----
# shinyApp(
#   ui = fluidPage(charDemographicsUI("charDemographics",show_filters = TRUE, data = charDemographics)),
